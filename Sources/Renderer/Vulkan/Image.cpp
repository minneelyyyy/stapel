...
{
    static VkImageView createImageView(VkDevice device, VkImage image, VkFormat format, VkImageViewType viewType, VkImageAspectFlags flags)
    {
        VkImageView view;
    ...
    Image::Image(VkDevice device, uint32_t width, uint32_t height, uint32_t depth, VkFormat format,
        VkImage image, VkImageLayout layout, VkImageView view)
        : device_(device), extent_({width, height, depth}), format_(format), image_(image),
        current_layout_(layout)
    {
        view_ = view ? view : createImageView(device_, image_, format_,
    ...
    Image::Image(VmaAllocator alloc, VkDevice device,
        ...
        VkFormat format)
        : allocator_(alloc), device_(device),
        extent_(width, height, depth), format_(format),
        current_layout_(VK_IMAGE_LAYOUT_UNDEFINED)
    {
        VkImageUsageFlags draw_img_use =
        ...
        view_ = createImageView(device_, image_, format_,
            VK_IMAGE_VIEW_TYPE_2D, VK_IMAGE_ASPECT_COLOR_BIT);
    }
    
    Image Image::Wrap(VkDevice device, uint32_t width, uint32_t height, uint32_t depth, VkFormat format,
        VkImage image, VkImageLayout layout, VkImageView view)
    {
    ...
    {
        VkImageAspectFlags aspectMask = (layout == VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL)
